...
{
	public static class DestroyableColliderBuilder
	{
		enum Edge
		{
			Left,
		...
		private static DestroyableLinkedNodes<Point> lines = new DestroyableLinkedNodes<Point>();
			...
			for (var y = MinY - 1; y <= MaxY; y++)
			{
				...
				for (var x = MinX - 1; x <= MaxX; x++)
				{
					...
					if (mask > 0 && mask < 15)
					{
						...
						switch (mask)
						{
							case 1: case 14: Link(0, 2, index - cellsWidth, index - 1); break; 
							case 2: case 13: Link(0, 3, index - cellsWidth, index + 1); break;
							case 4: case 11: Link(1, 2, index + cellsWidth, index - 1); break;
							case 8: case 7: Link(1, 3, index + cellsWidth, index + 1); break; 
							case 3: case 12: Link(2, 3, index - 1, index + 1); break;
							case 5: case 10: Link(0, 1, index - cellsWidth, index + cellsWidth); break; // L/R
							case 6: Link(1, 2, index + cellsWidth, index - 1); Link(0, 3, index - cellsWidth, index + 1); break;
							case 9: Link(0, 2, index - cellsWidth, index - 1); Link(1, 3, index + cellsWidth, index + 1); break;
						}
					}
				}
			}
			...
			for (var y = MinY - 1; y <= MaxY; y++)
			{
				...
				for (var x = MinX - 1; x <= MaxX; x++)
				{
					...
					if (mask > 0 && mask < 15)
					{
						...
						switch (mask)
						{
							case 1: case 14: Link(0, 2, cx, cy - 1, cx - 1, cy); break; 
							case 2: case 13: Link(0, 3, cx, cy - 1, cx + 1, cy); break;
							case 4: case 11: Link(1, 2, cx, cy + 1, cx - 1, cy); break;
							case 8: case 7: Link(1, 3, cx, cy + 1, cx + 1, cy); break; 
							case 3: case 12: Link(2, 3, cx - 1, cy, cx + 1, cy); break; 
							case 5: case 10: Link(0, 1, cx, cy - 1, cx, cy + 1); break; 
							case 6: Link(1, 2, cx, cy + 1, cx - 1, cy); Link(0, 3, cx, cy - 1, cx + 1, cy); break;
							case 9: Link(0, 2, cx, cy - 1, cx - 1, cy); Link(1, 3, cx, cy + 1, cx + 1, cy); break;
						}
					}
				}
			}
		...
		public static void BuildPoly(DestroyablePolygonColliderCell cell, Stack<PolygonCollider2D> tempColliders, GameObject child, float weld, float detail)
		{
